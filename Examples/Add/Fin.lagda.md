...
comb : ∀ {m k} → 𝔽 m × 𝔽 k → 𝔽 (k * m)
comb {m}{k} (i , j) = combine j i

-- comb-assoc : ∀ {m n k} {h : 𝔽 m}{i : 𝔽 n}{j : 𝔽 k}
--            → comb (comb (h , i) , j)
--            ≡ subst 𝔽 {!!} (comb (h , comb (i , j)))
-- comb-assoc {m}{n}{k}{h}{i}{j} =
--     begin
--       comb (comb (h , i) , j)
--     ≡⟨⟩
--       combine j (combine i h)
--     ≡⟨ {!!} ⟩
--       {!!}
...
toℕ-comb-assoc {m}{n}{k}{h}{i}{j} =
  ...
  ≡⟨ toℕ-comb (comb (h , i) , j) ⟩
    toℕ× (comb (h , i) , j)
  ≡⟨⟩
    toℕ (comb (h , i)) + toℕ j * (n * m)
  ≡⟨ cong (_+ toℕ j * (n * m)) (toℕ-comb (h , i)) ⟩
    toℕ× (h , i) + toℕ j * (n * m)
  ≡⟨⟩
    toℕ h + toℕ i * m + toℕ j * (n * m)
  ≡⟨ cong (toℕ h + toℕ i * m +_) (sym (*-assoc (toℕ j) n m)) ⟩
    (toℕ h + toℕ i * m) + toℕ j * n * m
  ≡⟨ +-assoc (toℕ h) (toℕ i * m) (toℕ j * n * m) ⟩
    toℕ h + (toℕ i * m + toℕ j * n * m)
  ≡⟨ sym (cong ((toℕ h) +_) (*-distribʳ-+ m (toℕ i) (toℕ j * n))) ⟩
...
Addᶜ⇉ {m} {μ = μ} = toℕ {2} ⊗ twice (toℕ ∘ μ) ⇉ toℕ ∘ comb {m}{2} ∘ first μ
...
infixr 2 _✯_
_✯_ : ∀ {m n}{r s} (μᵣ : r → 𝔽 m) (μₛ : s → 𝔽 n) → (r × s → 𝔽 (n * m))
μᵣ ✯ μₛ = comb ∘ (μᵣ ⊗ μₛ)


infixr 2 _×ᶜ_
_×ᶜ_ : ∀ {m n} {r s} {μᵣ : r → 𝔽 m}{μₛ : s → 𝔽 n}
     → Adder {m}{r}{μᵣ} → Adder {n}{s}{μₛ} → Adder {n * m}{r × s}{μᵣ ✯ μₛ}
...
-- toℕ-comb : ∀ {m k} → toℕ ∘ comb ≗ toℕ× {m}{k}
      ...
      begin
        ((toℕ ∘ comb ∘ first (μᵣ ✯ μₛ)) ∘ (+̂ᵣ ,̂ +̂ₛ)) (cᵢ , (xᵣ , xₛ) , (yᵣ , yₛ))
      ≡⟨⟩
        toℕ (comb (first (μᵣ ✯ μₛ) ((zᵣ , zₛ) , cₒ)))
      ≡⟨⟩
        toℕ (comb ((μᵣ ✯ μₛ) (zᵣ , zₛ) , cₒ))
      ≡⟨⟩
        toℕ (comb ((comb ∘ (μᵣ ⊗ μₛ)) (zᵣ , zₛ) , cₒ))
      ≡⟨⟩
        toℕ (comb (comb (μᵣ zᵣ , μₛ zₛ) , cₒ))
      ≡⟨ toℕ-comb-assoc {h = μᵣ zᵣ}{μₛ zₛ}{cₒ} ⟩
        toℕ (comb (μᵣ zᵣ , comb (μₛ zₛ , cₒ)))
      ≡⟨⟩
        toℕ (comb (first μᵣ (zᵣ , comb (first μₛ (zₛ , cₒ)))))
      ≡⟨⟩
        toℕ (comb (first μᵣ (zᵣ , comb (first μₛ (+̂ₛ (cₘ , xₛ , yₛ))))))
    ...
    -- commute : (toℕ ∘ comb ∘ first μ) ∘ +̂ ≗ addℕ ∘ (toℕ ⊗ twice (toℕ ∘ μ))
      ...
      -- ≡⟨⟩
      --   toℕ (comb (μᵣ zᵣ , μₛ zₛ)) + toℕ cₒ * (n * m)
      -- ≡⟨ cong (_+ toℕ cₒ * (n * m)) (toℕ-comb (μᵣ zᵣ , μₛ zₛ))⟩
      --   toℕ× (μᵣ zᵣ , μₛ zₛ) + toℕ cₒ * (n * m)
      -- ≡⟨ {!!} ⟩
      ...
      ≡⟨⟩
        toℕ cᵢ + (toℕ ∘ (μᵣ ✯ μₛ)) (xᵣ , xₛ) + (toℕ ∘ (μᵣ ✯ μₛ)) (yᵣ , yₛ)
      ≡⟨⟩
        addℕ ((toℕ ⊗ twice (toℕ ∘ (μᵣ ✯ μₛ))) (cᵢ , (xᵣ , xₛ) , (yᵣ , yₛ)))
      ≡⟨⟩
        (addℕ ∘ (toℕ ⊗ twice (toℕ ∘ (μᵣ ✯ μₛ)))) (cᵢ , (xᵣ , xₛ) , (yᵣ , yₛ))
      ∎

     -- begin
     --   ((toℕ× ∘ first (μᵣ ✯ μₛ)) ∘ (+̂ᵣ ,̂ +̂ₛ)) (cᵢ , (xᵣ , xₛ) , (yᵣ , yₛ))
     -- ≡⟨⟩
     --   (toℕ× ∘ first (μᵣ ✯ μₛ)) ((zᵣ , zₛ) , cₒ)
     -- ≡⟨⟩
     --   toℕ× ((μᵣ ✯ μₛ) (zᵣ , zₛ) , cₒ)
     -- ≡⟨⟩
...
--   mk {mᵣ * mₛ} {r × s} {μᵣ ✯ μₛ} (+̂ᵣ ,̂ +̂ₛ) λ (cᵢ , (xᵣ , xₛ) , (yᵣ , yₛ)) → ?
...
_✯_ : ∀ {m n}{r s} (μᵣ : r → 𝔽 m) (μₛ : s → 𝔽 n) → (r × s → 𝔽 (n * m))
μᵣ ✯ μₛ = comb ∘ (μᵣ ⊗ μₛ)
-- (μᵣ ✯ μₛ) (zᵣ , zₛ) = combine (μᵣ zᵣ) (μₛ zₛ)
...
--     in (zᵣ , zₛ) , cₒ
    ...
    -- (μᵣ ✯ μₛ) (xᵣ , xₛ) = combine (μᵣ xᵣ) (μₛ xₛ)
...
-- Goal: (toℕ× ∘′ first (μᵣ ✯ μₛ)) ∘′ (+̂ᵣ ,̂ +̂ₛ) ≗
--       addℕ ∘′ (toℕ ⊗ twice (toℕ ∘′ (μᵣ ✯ μₛ)))
-- ————————————————————————————————————————————————————————————
-- +̃ₛ : (toℕ× ∘′ first μₛ) ∘′ +̂ₛ ≗
...
How can we combine them into a digit adder for base `n * m`?
