module Examples.Add.MFin where
...
addᶜ⇉ : ∀ k {r : Set} {μ : r → ℕ} (+̂ : C r)
      → combℕ k ∘ (μ ⊗ bval) ∘ +̂ ≗ addℕ ∘ (bval ⊗ twice μ)
      → bval ⊗ twice μ ⇉ combℕ k ∘ (μ ⊗ bval)
addᶜ⇉ k +̂ = mk +̂ addℕ

record Adder k {r : Set}{μ : r → ℕ} : Set where
  constructor mk
  field
    arr : bval ⊗ twice μ ⇉ combℕ k ∘ (μ ⊗ bval)
    ⦃ isAdd ⦄ : f₂ arr ≡ addℕ

adder : ∀ {k} {r : Set} {μ : r → ℕ} (+̂ : C r)
      → combℕ k ∘ (μ ⊗ bval) ∘ +̂ ≗ addℕ ∘ (bval ⊗ twice μ)
      → Adder k {r}{μ}
...
1ᶜ = adder +̂ comm
 where
   ...
   comm : combℕ 2 ∘ (bval ⊗ bval) ∘ +̂ ≗ addℕ ∘ (bval ⊗ twice bval)
   comm (𝕗 , 𝕗 , 𝕗) = refl
   comm (𝕗 , 𝕗 , 𝕥) = refl
   comm (𝕗 , 𝕥 , 𝕗) = refl
   comm (𝕗 , 𝕥 , 𝕥) = refl
...
infixr 2 _✯_
_✯_ : ∀ {r s} (μᵣ : r → ℕ) (μₛ : s → ℕ) → (kᵣ : ℕ) → (r × s → ℕ)
(μᵣ ✯ μₛ) kᵣ (xᵣ , xₛ) = combℕ kᵣ (μₛ xₛ , μᵣ xᵣ)
...
infixr 2 _×ᶜ_
_×ᶜ_ : ∀ {kᵣ kₛ} {r s} {μᵣ : r → ℕ}{μₛ : s → ℕ}
     → Adder kᵣ {r}{μᵣ} → Adder kₛ {s}{μₛ} → Adder (kᵣ * kₛ) {r × s}{(μᵣ ✯ μₛ) kᵣ}

_×ᶜ_ {kᵣ}{kₛ}{r}{s}{μᵣ}{μₛ} (adderᵖ +̂ᵣ +̃ᵣ) (adderᵖ +̂ₛ +̃ₛ) =
...
--       -- begin
--       --   ((comb₂ ∘ first (μᵣ ✯ μₛ)) ∘ (+̂ᵣ ,̂ +̂ₛ)) (cᵢ , (xᵣ , xₛ) , (yᵣ , yₛ))
--       -- ≡⟨⟩
--       --   comb₂ (combine (μₛ zₛ) (μᵣ zᵣ) , cₒ)
--       -- ≡⟨ {!!} ⟩
--       --   add𝔽 (bval cᵢ , combine (μₛ xₛ) (μᵣ xᵣ) , combine (μₛ yₛ) (μᵣ yᵣ))
--       -- ≡⟨⟩
--       --   (add𝔽 ∘ (bval ⊗ twice (μᵣ ✯ μₛ))) (cᵢ , (xᵣ , xₛ) , (yᵣ , yₛ))
--       -- ∎

-- Can I stop using quotRemℕ and addcℕ altogether?
